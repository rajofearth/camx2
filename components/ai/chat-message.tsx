 ...
 */
function ChatMessage({ role, timestamp, sender, children, className, ...props }: ChatMessageProps) {
...
function VlmBullet({ variant = "default", className, children, ...props }: VlmBulletProps) {
  const prefixColor = {
...
interface VlmDataTableProps {
  headers: string[]
  rows: Array<{ cells: React.ReactNode[]; variant?: "default" | "warning" | "critical" }>
}
        ...
        return (
          ...
          >
            {row.cells.map((cell, ci) => (
              <div key={ci} className={ci === row.cells.length - 1 ? valueColor : ""}>
                {cell}
              </div>
            ))}
          </div>
...
function AggregateConfidence({ value }: { value: number }) {
  return (
    <div className="mt-4 flex items-center gap-2">
      <span className="font-mono text-[10px] text-op-text-sec">AGGREGATE CONFIDENCE</span>
      <div className="h-0.5 flex-1 bg-op-text-muted">
        <div
          className="h-full bg-op-silver"
          style={{ width: `${value}%` }}
        />
      </div>
